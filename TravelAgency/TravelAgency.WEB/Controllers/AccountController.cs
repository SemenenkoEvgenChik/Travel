...
namespace TravelAgency.WEB.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    if (rolename == null)
                    {
                        int status = await _customerService.CheckStatusCustomer(user.Id);
        ...
        {
            var user = await _customerService.Register(model);
            if (user != null)
            {
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            string[] userInf = loginInfo.ExternalIdentity.Name.Split(' ');
            if (userInf.Length < 2)
            {
                throw new System.Exception();
            }
            string name = userInf[0];
            string surname = userInf[1];
            ...
            if (user != null)
            {
                status = await _customerService.CheckStatusCustomer(user.Id);
            }
            if (status == 2)
            {
                AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                string[] userInf = info.ExternalIdentity.Name.Split(' ');
                if (userInf.Length < 2)
                {
                    throw new System.Exception();
                }
                string name = userInf[0];
                string surname = userInf[1];
                await _customerService.ExternalRegisterCustomer(name, surname, user.Id);
        ...
        {
            ModelStateView result = new ModelStateView();
            result.StateResult = await _customerService.ChangePassword(request, User.Identity.GetUserId());
